...
elseif($method == 'approval_note_func'){
        ...
        foreach($stmt->fetchALL() as $x){
            ...
            $compatible = $step + 1;
            if($compatible == $level){
                $step_two_appr = "UPDATE tb_request_mp SET step = '3',approve_noted_by = '$name',approve_check_remarks = 'APPROVED',approval_status = 'FOR APPROVAL OF RECRUITMENT ASST. MNGR.' WHERE id = '$id'";
                $stmt = $conn->prepare($step_two_appr);
                if($stmt->execute()){
                    echo 'true';
                }else{
