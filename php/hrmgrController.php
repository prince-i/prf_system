...
if($method == 'for_approval'){
    $filter = $_POST['filter'];
    if(empty($filter)){
        $fetch = "SELECT id,requestor,requesting_position,assigned_dept,contract_status,requestor_email,approval_status,verification_status,request_date FROM tb_request_mp WHERE  step ='4'  ORDER BY id DESC";
        ...
        $fetch = "SELECT id,requestor,requesting_position,assigned_dept,contract_status,requestor_email,approval_status,verification_status,request_date FROM tb_request_mp WHERE  step ='4' AND assigned_dept LIKE '$filter%' ORDER BY id DESC";
        $stmt=$conn->prepare($fetch);
        $stmt->execute();
        foreach($stmt->fetchALL() as $x){
            echo '<tr class="modal-trigger" data-target="universal" onclick="get_id(&quot;'.$x['id'].'&quot;)">';
...
elseif($method == 'pending_view'){
    $filter = $_POST['filter'];
    if(empty($filter)){
        $fetch = "SELECT id,requestor,requesting_position,assigned_dept,contract_status,requestor_email,approval_status,verification_status,request_date FROM tb_request_mp WHERE  step > 4 AND step < 7  ORDER BY id DESC";
        $stmt=$conn->prepare($fetch);
        $stmt->execute();
        foreach($stmt->fetchALL() as $x){
        ...
        $fetch = "SELECT id,requestor,requesting_position,assigned_dept,contract_status,requestor_email,approval_status,verification_status,request_date FROM tb_request_mp WHERE step > 4 AND step < 7 AND assigned_dept LIKE '$filter%' ORDER BY id DESC";
        $stmt=$conn->prepare($fetch);
        $stmt->execute();
        foreach($stmt->fetchALL() as $x){
            echo '<tr class="modal-trigger" data-target="pendingModal" onclick="preview_pending(&quot;'.$x['id'].'&quot;)">';
