<?php
    ...
    elseif($method == 'approve_hrd'){
        ...
        if($step == 4){
            $compatible = $step + 2;
            if($compatible == $level){
                // APPROVING QUERY HRD MNGR AND HRD DIV MGR WILL BE THE ONE WHO APPROVE 
                $approveQL = "UPDATE tb_request_mp SET verify_verifier_div_mgr = '$name', verify_verifier_div_mgr_remarks = 'APPROVED', verification_status = 'FOR PRESIDENT APPROVAL', step = '6',verify_verifier_manager = '$name',verify_verifier_manager_remarks = 'APPROVED' WHERE id = '$id'";
        ...
        if($step == 5){
            ...
            if($compatible == $level){
                // APPROVING QUERY HRD MNGR AND HRD DIV MGR WILL BE THE ONE WHO APPROVE 
                $approveQL = "UPDATE tb_request_mp SET verify_verifier_div_mgr = '$name', verify_verifier_div_mgr_remarks = 'APPROVED', verification_status = 'FOR PRESIDENT APPROVAL', step = '6' WHERE id = '$id'";
                $stmt = $conn->prepare($approveQL);
                if($stmt->execute()){
                    echo 'success';
                }else{
    ...
    elseif($method == 'declineBy_hrd'){
        ...
        $decline_step = 0;
        $decline_query = "UPDATE tb_request_mp SET verify_verifier_div_mgr_remarks = '$decline_stmt',step = '$decline_step',verify_verifier_manager_remarks = '$decline_stmt', verification_status = 'DISAPPROVED' WHERE id = '$prf_id'";
